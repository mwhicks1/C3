...
node_t * * make_table(int size, int procname)  {
  node_t * * retval  = calloc<node_t *>(size, sizeof(node_t *));
  assert(retval);
  return retval;
}
		    ...
		    int degree, int percent_local, int id)
{
  ...
  for(; cur_node; cur_node=cur_node->next) {
    ...
    node_t * * tmp  = calloc<node_t *>(degree, (sizeof(node_t *)));
    ;
    ...
    for (j=0; j<degree; j++) {
      do {
        ...
        node_t * * ub = tmp + j;
        UncheckedBoundsInit(node_t * *, tmp2, bounds(tmp2, ub), tmp);
        for ( ; tmp2 < ub; tmp2++)
...
void update_from_coeffs(node_t * nodelist) {
  ...
  for (; cur_node; cur_node=cur_node->next) {
    ...
    if (from_count < 1) {
      ...
      cur_node->from_values = calloc<double *>(20, sizeof(double *));
      cur_node->coeffs = calloc<double>(20, sizeof(double));
      cur_node->from_length = 0;
    } else {
      cur_node->from_values = calloc<double *>(from_count, sizeof(double *));
...
void make_all_neighbors(table_t * table,int groupname) {
  ...
  node_t * * local_table  = NULL;
  table_arr_t * local_table_array  = NULL;
